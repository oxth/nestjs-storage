...
interface StoredFilesFields {
  [key: string]: StoredFile[];
}

export function StorageFilesFieldsInterceptor(
  fieldNames: string[],
  maxCount?: number,
  ...
  class MixinInterceptor implements NestInterceptor {
    ...
    async intercept(
      ...
      const storedFileFields: StoredFilesFields = fieldNames.reduce(
        (acc, fieldName) => {
          acc[fieldName] = [];
          return acc;
        },
    ...
    private runMulter(req: Request, res: Response): Promise<void> {
      return new Promise<void>((resolve, reject) => {
        this.multer.fields(
          fieldNames.map((field) => ({ name: field, maxCount })),
        )(req, res, (err: any) => {
          if (err) {
